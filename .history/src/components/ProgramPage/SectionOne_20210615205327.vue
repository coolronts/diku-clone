<template>
  <div :class="style.main">
    ...
    <p :class="[style.deadline,{slash:over}]"> Application deadline: <span class="text-black"> {{date}} within 12:00 PM </span> </p>
  </div>  
</template>
...
<script>
  ...
  export default {
    ...
    mounted(){
      ...
      .then(res => res.json())
      .then(data => data.find(element => element.id == this.$route.params.id))
      .then(found => this.programData = found.id)
